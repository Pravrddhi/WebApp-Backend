...
class UserRegistration extends Component {
  ...
  checkIfUserExists = async (mobileNo) => {
    try {
      const response = await axios.post('http://localhost:8000/api/users/existing_user/', {
        mobile_number: mobileNo,
      });
      return response.data.existing_user;
    ...
    if (this.validateForm()) {
      ...
      try {
        ...
        if (!response || response.status !== 201) {
          this.showPopup(response?.data?.message || 'Registration failed. Please try again.');
          return;
        }
